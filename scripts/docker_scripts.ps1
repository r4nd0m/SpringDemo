...
docker buildx b -f Dockerfile --build-arg JAR_FILE=build/libs/SpringDemo-0.0.1-SNAPSHOT.jar -t kalenyuk/spring-demo .
...
docker run -p 8080:8080 --network=local-docker-network --name springboot-demo kalenyuk/spring-demo --attach